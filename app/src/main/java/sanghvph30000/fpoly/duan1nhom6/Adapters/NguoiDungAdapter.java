...
import sanghvph30000.fpoly.duan1nhom6.DAO.NguoiDungDAO;
import sanghvph30000.fpoly.duan1nhom6.Models.NguoiDung;
...
public class NguoiDungAdapter extends RecyclerView.Adapter<NguoiDungAdapter.MyViewHover> {
    ...
    public void onBindViewHolder(@NonNull MyViewHover holder, int position) {
        ...
        if (isUri) {
            ...
            // Nếu không phải là đường dẫn URI, kiểm tra tính hợp lệ của tài nguyên drawable
            int resourceId = context.getResources().getIdentifier(imgSrc, "drawable", context.getPackageName());
            if (resourceId != 0) {
                // Nếu tài nguyên drawable tồn tại, sử dụng Picasso để tải ảnh từ nguồn drawable
                Picasso.get().load(resourceId).transform(new CircleTransform()).into(holder.imgNguoiDung);
            } else {
                // Xử lý trường hợp tài nguyên không tồn tại (ví dụ: hiển thị một hình ảnh mặc định)
                 holder.imgNguoiDung.setImageResource(R.drawable.avatar_mac_dinh);
            }
        }
